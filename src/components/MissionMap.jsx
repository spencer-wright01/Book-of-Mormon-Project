    ...
    <section className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-8 grid gap-5 lg:grid-cols-[1.25fr_0.75fr]">
          ...
          <div className="relative flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div className="space-y-3">
              ...
              <p className="max-w-3xl text-lg leading-8 text-slate-200">
                Your custom hero is the one crossing this starlane. Each mission teaches through Book of Mormon examples, but it is your character who earns the armor, charges the power-ups, and faces Darth Korvax.
              </p>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="rounded-[1.5rem] border border-white/10 bg-slate-950/55 p-4">
                <p className="text-sm uppercase tracking-[0.2em] text-slate-400">Hero Status</p>
                <p className="mt-2 font-display text-2xl text-white">{player?.title}</p>
                <p className="mt-2 text-slate-300">Blade Color: {player?.lightsaberColor}</p>
                <div className="mt-4">
        ...
        <div className="glass-panel rounded-[2rem] border border-white/10 p-6">
          <p className="font-display text-sm uppercase tracking-[0.3em] text-cyan-200/80">Starlane Progress</p>
          <div className="mt-5 space-y-4">
            {missions.map((mission, index) => (
              <div key={mission.id} className="flex items-center gap-4">
                <div className="flex flex-col items-center">
                  <div className="status-ring flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-slate-900 font-display text-lg text-white">
                    {index + 1}
                  </div>
                  {index < missions.length - 1 ? (
                    <div className="h-8 w-px bg-gradient-to-b from-cyan-300/70 to-transparent" />
                  ) : null}
                </div>
                <div>
                  <p className="font-semibold text-white">{mission.title}</p>
                  <p className="text-sm text-slate-300">Status: {missionStatuses[mission.id]}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1.35fr_0.65fr]">
        <div className="glass-panel rounded-[2rem] border border-amber-200/20 p-6 shadow-xl">
          ...
          <p className="mt-3 text-slate-200 leading-7">
            The final battle unlocks only after all six missions are completed and every power-up is
            charged.
          </p>
        ...
        <div className="glass-panel rounded-[2rem] border border-white/10 p-6">
          <p className="font-display text-sm uppercase tracking-[0.3em] text-cyan-200/80">Progress</p>
          <dl className="mt-4 space-y-4">
            <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-4">
              <dt className="text-sm uppercase tracking-[0.2em] text-slate-400">Completed Missions</dt>
              <dd className="mt-2 font-display text-3xl text-white">{player.completedMissions.length}/6</dd>
            </div>
            <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-4">
              <dt className="text-sm uppercase tracking-[0.2em] text-slate-400">Charged Power-Ups</dt>
              <dd className="mt-2 font-display text-3xl text-white">{player.chargedPowerUps.length}/6</dd>
            </div>
            <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-4">
              <dt className="text-sm uppercase tracking-[0.2em] text-slate-400">Active Hero</dt>
              <dd className="mt-2 text-lg text-white">{player.title} with a {player.lightsaberColor} saber</dd>
            </div>
          </dl>
        </div>
      </div>
